export {};
// ============================================================
// Lecture 1 — Wrap-up Demo: MUDIK PLANNER
// Plan your Lebaran trip back to your hometown
// ============================================================

// --- UNION TYPES ---
type Transport = "motor" | "mobil" | "kereta" | "pesawat" | "bus";
type Province = "Jawa Barat" | "Jawa Tengah" | "Jawa Timur" | "Sumatera" | "Kalimantan" | "Sulawesi";
type Status = "belum berangkat" | "dalam perjalanan" | "sudah sampai";
...
let checkpoint: [string, number, number] = ["Cirebon", 258, 3];
console.log(`Checkpoint: ${checkpoint[0]} — ${checkpoint[1]} km dari Jakarta (~${checkpoint[2]} jam)`);

// --- NUMBERS ---
let totalPesanaMudik: number = 26_500_000; // mudik travellers nationally
let hargaTiketKereta: number = 350_000;
let hargaBensin: number = 10_000; // per liter

// --- BOOLEANS ---
let isMacetParah: boolean = true;
let isH1Lebaran: boolean = false;
...
function estimasiWaktu(jarak: number, transport: Transport): string {
  const kecepatan: { [key: string]: number } = {
...
function umumkanStatus(nama: string, tujuan: string, status: Status): void {
  const icon =
    status === "sudah sampai"     ? "✅" :
...
function pesanTiket(tujuan: string, transport: Transport, kursi?: string): string {
  return kursi
...
function biayaBBM(jarak: number, konsumsi: number = 20): number {
